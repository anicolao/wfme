<script lang="ts">
  ...
  export let onCompleteWarEffort: (definitionId: string) => void;
...
<section class="table" aria-labelledby="table-title">
  ...
  {#if game.match?.warEffortsEnabled}
    <section class="market war-efforts" data-testid="war-efforts" aria-labelledby="war-effort-title">
      <div>
        <p class="eyebrow">Optional public module</p>
        <h2 id="war-effort-title">War Efforts</h2>
        <p>{game.match.warEffortDeck.length} facedown · {game.match.warEffortDiscard.length} discarded</p>
      </div>
      <div class="chronicle-row" aria-label="Face-up War Efforts">
        {#each game.match.warEffortRow as instance}
          {@const effort = WAR_EFFORT_DEFINITIONS.find((definition) => definition.id === instance.definitionId)!}
          <article data-testid={`war-effort-${effort.id}`}><strong>{effort.name}</strong><span>{effort.completionText}</span><span>{effort.rewardText}</span></article>
        {/each}
      </div>
      <div class="reserve" aria-label="Held War Efforts">
        {#each game.players as player}
          {@const held = game.match.players[player.uid].warEffort}
  ...
  {#if game.match?.pendingChoice && game.match.pendingChoice.kind !== 'place-scout' && game.match.pendingChoice.kind !== 'fell-sorcery' && game.match.pendingChoice.kind !== 'token-command-order' && game.match.pendingChoice.kind !== 'commander-ring-standing' && game.match.pendingChoice.kind !== 'commander-ring-gandalf' && game.match.pendingChoice.kind !== 'commander-ring-eowyn' && game.match.pendingChoice.kind !== 'commander-engines-isengard' && game.match.pendingChoice.kind !== 'commander-black-breath' && game.match.pendingChoice.kind !== 'commander-fate-foresight'}
    <section class="pending-choice" data-testid="pending-choice" aria-labelledby="choice-title">
      ...
      <div class="choice-actions">
        {#if game.match.pendingChoice.kind === 'take-war-effort'}
          {#each game.match.pendingChoice.instanceIds as instanceId}
            {@const instance = game.match.warEffortRow.find((candidate) => candidate.id === instanceId)}
            {@const effort = WAR_EFFORT_DEFINITIONS.find((definition) => definition.id === instance?.definitionId)}
            <button type="button" disabled={game.match.pendingChoice.actorUid !== localUid} onclick={() => onResolveChoice(`take-war-effort:${instanceId}`)}>Take {effort?.name ?? 'War Effort'}</button>
          {/each}
          <button type="button" disabled={game.match.pendingChoice.actorUid !== localUid} onclick={() => onResolveChoice('decline-war-effort')}>Leave the War Efforts</button>
        {:else if game.match.pendingChoice.kind === 'critical-defense'}
