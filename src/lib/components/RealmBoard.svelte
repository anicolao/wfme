...
<section class="table" aria-labelledby="table-title">
  <header class="table-header">
    ...
    <dl class="ledger" aria-label="Construction capability ledger">
      ...
      <div><dt>Chronicle cards</dt><dd>38 / 54</dd></div>
      ...
      <div class="choice-actions">
        ...
        {:else if game.match.pendingChoice.kind === 'chronicle-card-choice'}
          {#each game.match.pendingChoice.cardInstanceIds as cardId}
            ...
            {@const prefix = game.match.pendingChoice.definitionId === 'ranger-north' ? 'discard-card:' : 'trash-card:'}
            <button type="button" disabled={game.match.pendingChoice.actorUid !== localUid} onclick={() => onResolveChoice(`${prefix}${cardId}`)}>{game.match.pendingChoice.definitionId === 'ranger-north' ? 'Discard' : 'Trash'} {card ? cardName(card.definitionId) : 'private card'}</button>
          {/each}
