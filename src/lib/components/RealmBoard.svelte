<script lang="ts">
  ...
  $: tokenCommander = game.match?.pendingChoice?.kind === 'token-command-order'
    ...
    : undefined;
  $: tokenRingName = tokenCommander === 'theoden' ? 'Ride Now' : tokenCommander === 'galadriel' ? 'Mirror Unveiled' : tokenCommander === 'gandalf' ? 'Kindle Courage' : tokenCommander === 'eowyn' ? 'Choose Deeds' : 'Andúril Aflame';
