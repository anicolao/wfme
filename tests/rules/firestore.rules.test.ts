...
describe('integrated game event security rules', () => {
  ...
  it('allows authenticated, attributed, append-only events', async () => {
    ...
    for (const [index, type] of ['game/war-efforts-set', 'game/rivals-set', 'turn/revealed', 'card/acquired', 'reveal/finished', 'scout/placed', 'battle/passed', 'fate/played', 'endgame/passed', 'match/rematch-ready'].entries()) {
    ...
    await assertFails(setDoc(
      doc(db, 'games/RIVEN/events/player-a-000013'),
      validEvent({ id: 'player-a-000013', type: 'client/bypassed-choice', clientSeq: 13 })
    ));
  });
