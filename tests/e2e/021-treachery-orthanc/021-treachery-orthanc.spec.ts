...
test('Treachery at Orthanc resolves a private ranked Fate choice before Recall', async ({ browser, page }, testInfo) => {
  ...
  try {
        ...
        await steps.gesture(actor.page, `round-${round}-finish-${turn + 1}`, `${actor.name} finishes Reveal in round ${round}`, async () => {
          ...
          { spec: turn < 2 ? 'Reveal authority advances clockwise' : 'Treachery at Orthanc opens as the first selected Age II Battle', check: async () => {
            if (turn < 2) await expect(actor.page.locator('footer')).not.toContainText(`Current actor ${actor.name}`);
              ...
              await expect(observer.page.getByTestId('active-battle')).toContainText('Second: draw 2 Fate, keep 1');
              await expect(observer.page.getByTestId('active-battle')).toContainText('Third: draw 1 Fate');
            }
          } },
          converged(accepted.value + 1)
        ]);
    ...
    for (let index = 0; index < 2; index += 1) {
      ...
      await steps.gesture(actor.page, `treachery-${index + 1}-choose-card`, `${actor.name} chooses a real Battle card`, async () => {
      ...
      await steps.gesture(actor.page, `treachery-${index + 1}-enter`, `${actor.name} enters the Treachery through ${placement.destinationName}`, async () => {
      ...
      await finishOrderedPlacementChoices(actor, `treachery-${index + 1}`);
      const amount = index === 0 ? 2 : 1;
      await steps.gesture(actor.page, `treachery-${index + 1}-deploy`, `${actor.name} deploys ${amount} ${amount === 1 ? 'Company' : 'Companies'}`, async () => {
    ...
    for (let turn = 0; turn < 3; turn += 1) {
      const actor = await currentSeat();
      await steps.gesture(actor.page, `treachery-reveal-${turn + 1}`, `${actor.name} Reveals for the Treachery`, async () => {
        await actor.page.getByRole('button', { name: 'Reveal remaining hand' }).click(); accepted.value += 1;
      }, [
      ...
      await steps.gesture(actor.page, `treachery-finish-${turn + 1}`, `${actor.name} finishes the Treachery Reveal`, async () => {
        await actor.page.getByRole('button', { name: 'Finish Reveal' }).click(); accepted.value += 1;
      }, [
    ...
    const winnerRenownBefore = winner ? await count(seats[0], winner.name, 'Renown') : -1;
    const winnerShadowBefore = winner ? await count(seats[0], winner.name, 'Shadow') : -1;
    const fateBefore = Object.fromEntries(await Promise.all(rewardRecipients.map(async (recipient) => [recipient.name, await count(seats[0], recipient.name, 'Fate')])));
    const discardBefore = Number((await page.getByTestId('fate-discard').textContent())?.match(/(\d+)/)?.[1] ?? '-1');

    for (let pass = 0; pass < 2; pass += 1) {
      const actor = await currentSeat();
      await steps.gesture(actor.page, `treachery-pass-${pass + 1}`, `${actor.name} passes Combat Fate`, async () => {
        await actor.page.getByTestId('pass-battle').click(); accepted.value += 1;
      }, [
        { spec: pass === 0 ? 'Combat authority passes to the other participant' : 'Ranked rewards resolve and the private Fate choice blocks Recall', check: async () => {
    ...
    for (const seat of seats) if (await seat.page.getByRole('button', { name: 'Keep Fate 1' }).isEnabled()) choiceActor = seat;
    if (!choiceActor) throw new Error('No ranked human owns the Treachery Fate choice');
    await steps.gesture(choiceActor.page, 'reload-treachery-fate', `${choiceActor.name} reloads the private ranked Fate choice`, async () => {
      await reloadGameClient(choiceActor!.page);
    }, [
      { spec: 'The exact recipient and two private choices replay before Recall', check: async () => {
        await expect(choiceActor!.page.getByRole('heading', { name: 'Keep one of the two Fate cards?' })).toBeVisible();
        await expect(choiceActor!.page.getByRole('button', { name: 'Keep Fate 1' })).toBeEnabled();
        await expect(choiceActor!.page.getByRole('button', { name: 'Keep Fate 2' })).toBeEnabled();
        for (const observer of seats.filter((seat) => seat !== choiceActor)) {
          await expect(observer.page.getByRole('button', { name: 'Keep Fate 1' })).toBeDisabled();
          await expect(observer.page.getByRole('button', { name: 'Keep Fate 2' })).toBeDisabled();
        }
      } },
    ...
    for (let index = 0; index < rewardRecipients.length; index += 1) {
      ...
      for (const seat of seats) if (await seat.page.getByRole('button', { name: 'Keep Fate 1' }).isEnabled()) recipient = seat;
      if (!recipient) throw new Error(`Fate reward ${index + 1} has no authorized human`);
      await steps.gesture(recipient.page, `keep-treachery-fate-${index + 1}`, `${recipient.name} privately keeps Fate card one`, async () => {
        await recipient!.page.getByRole('button', { name: 'Keep Fate 1' }).click(); accepted.value += 1;
      }, [
        { spec: 'The ranked recipient keeps exactly one Fate and publicly discards the other', check: async () => {
          for (const observer of seats) {
            await expect(row(observer, recipient!.name).getByText('Fate', { exact: true }).locator('..')).toContainText(String(fateBefore[recipient!.name] + 1));
