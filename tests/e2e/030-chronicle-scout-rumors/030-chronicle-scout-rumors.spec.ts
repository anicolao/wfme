...
test('Whispered Rumor and Goblin Informer execute both Scout-linked boxes', async ({ browser, page }, testInfo) => {
  ...
  const table = await startPlotTable(browser, page, testInfo, steps, 'chronicle-scout-rumors-9', { phone: 'RUMRP', desktop: 'RUMRD' });
  ...
  try {
    ...
    for (const observer of seats) {
      ...
      await expect(observer.page.getByText('Chronicle cards').locator('..')).toContainText('34 / 54');
    }
    ...
    for (let guard = 0; guard < 220 && !(rumorMuster && goblinMuster && goblinJourney && rumorPlacements >= 2); guard += 1) {
      ...
      if (actor === buyer) {
        ...
        if (!goblinMuster && hasGoblin && hasScout) {
          await reveal(buyer);
